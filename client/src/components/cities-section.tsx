        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-4">
          {displayCities.map((city, index) => (
            <motion.div
              ...
              className="group relative rounded-xl border border-[#00C2FF]/15 bg-[#0d2255]/40 p-5 text-center hover:border-[#00C2FF]/40 hover:shadow-[0_0_20px_rgba(0,194,255,0.1)] transition-all duration-300 cursor-pointer"
              data-testid={`card-city-${index}`}
            >
              <MapPin className="w-6 h-6 text-[#00C2FF] mx-auto mb-2 group-hover:text-[#00FFE0] transition-colors duration-300" />
              <h3 className="text-white font-bold text-base">{city.name}</h3>
              <p className="text-[#EAF7FF]/40 text-xs mt-1">{city.state}</p>
            </motion.div>
          ))}
        </div>
      </div>
